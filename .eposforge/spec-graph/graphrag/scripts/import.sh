...
# Parquet files from installed/spec-graph/graphrag/output/ and batch-imports them into Neo4j.
...
#   - Python venv at installed/spec-graph/graphrag/.venv with neo4j, pandas, pyarrow installed.
...
#   bash.eposforge/spec-graph/graphrag/scripts/import.sh
set -euo pipefail
...
if [[ ! -d "${OUTPUT_DIR}" ]]; then
  echo "ERROR: GraphRAG output directory not found: ${OUTPUT_DIR}" >&2
    echo "Run.eposforge/spec-graph/graphrag/scripts/index.sh first." >&2
  exit 1
fi
