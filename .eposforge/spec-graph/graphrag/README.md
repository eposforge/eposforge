...
GraphRAG workspace only for explicit fallback runs (full nuke-and-reproject
into a separate Neo4j instance).

See [`graphrag.md`](./graphrag.md) for the adapter's Living Spec, and
...
After a full rebuild (`bash.eposforge/spec-graph/graphrag/scripts/rebuild.sh`), confirm
