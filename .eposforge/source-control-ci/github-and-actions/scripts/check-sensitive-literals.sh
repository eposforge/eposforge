...
# Note: "GEA" treated as short identifier for the (sanitized) primary adopter example; always use generics in public content.
...
scan_file() {
  ...
  if grep -Iq . "${file_path}"; then
    ...
    # Also tolerate explicit mentions inside docs that describe the rule / give the verification rg command
    ...
    if [[ -n "${adopter_hits}" ]]; then
      # Filter hits that are part of rule documentation or pattern definition
      adopter_hits=$(printf '%s\n' "${adopter_hits}" | grep -vE '(PRIVATE_ADOPTER_PATTERN|rg ".*(Grace|GEA)|forbidden in public|adopter name matches|specific adopter repository names|Use `rg|must never appear)' || true)
    fi
