...
#  .eposforge/<component>/scripts/hooks/<git-hook-name>
#  .eposforge/<component>/<adapter>/scripts/hooks/<git-hook-name>
#
...
INSTALLED_ROOT="${REPO_ROOT}/.eposforge"  # flat layout (component dirs directly under.eposforge/)
...
#.eposforge/*/scripts/hooks/ and.eposforge/*/*/scripts/hooks/.
...
status=0
for fragment in \
  "$repo_root"/.eposforge/*/scripts/hooks/"$hook_name" \
  "$repo_root"/.eposforge/*/*/scripts/hooks/"$hook_name"; do
  if [ -x "$fragment" ]; then
    "$fragment" "$@"
  else
    bash "$fragment" "$@"
...
case "$MODE" in
  install)
    ...
    discover_fragments | awk -F'\t' \
      '{ sub(/^.*\/\.eposforge\//, ".eposforge/", $2); printf "  %s\t%s\n", $1, $2 }'
    ;;
  check)
