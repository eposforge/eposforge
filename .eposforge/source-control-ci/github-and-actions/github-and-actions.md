...
| `status` | `approved` |
> Slot contract: [../../../01-architecture/02-components/source-control-ci.md](../../../01-architecture/02-components/source-control-ci.md)
...
bash.eposforge/source-control-ci/github-and-actions/scripts/install-hooks.sh --check

# Remove all managed dispatchers
bash.eposforge/source-control-ci/github-and-actions/scripts/install-hooks.sh --uninstall
```
...
if any files exist under `.eposforge/scripts/`. The rule is: adapter scripts
must live under `.eposforge/<component>/scripts/`. The same check is
