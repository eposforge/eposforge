...
> `.eposforge/spec-graph/` must update this file in the same commit.
...
entities and relationships into its embedded Ladybug graph (Kuzu fork)
+ LanceDB vector store, hosted inside the `dkr-cgnee-api` container.
Microsoft GraphRAG is retained only as an archived revival-only snapshot
under `.eposforge/spec-graph/graphrag/`; it is not part of the active
path and should not be used to infer current EposForge behavior.
...
Per [../01-architecture/00-adapter-pattern/adapter-pattern.md](../01-architecture/00-adapter-pattern/adapter-pattern.md)
and [../01-architecture/02-components/spec-graph.md](../01-architecture/02-components/spec-graph.md):
...
| `version` | `0.1.1` |
| `privacy_posture` | `local` (embedded Ladybug/Kuzu-fork + LanceDB store inside `dkr-cgnee-api`) / `vendor-default` (inference during indexing) |
...
| `microsoft-graphrag` | `SPEC_GRAPH` | shelved | `bash.eposforge/spec-graph/graphrag/scripts/rebuild.sh` | Archived GraphRAG snapshot retained only for possible revival. Not part of the active path. |
| `neo4j-ce` | `SPEC_GRAPH` | shelved | `.eposforge/spec-graph/graphrag/scripts/import.sh` + Neo4j MCP | Archived GraphRAG target store only; not used by the active Cognee path. |
| `file-based-backlog` | `BACKLOG` | implemented, experimental | `bash.eposforge/backlog/file-based-backlog/scripts/{new-issue,lint-backlog,sweep-resolved,aggregate}.sh` | Local markdown backlog adapter with cross-repo aggregation and archive indexing |

Candidate adapters remain cataloged in
`../03-research/01-architecture/02-components/spec-graph/spec-graph.md` and are non-normative until listed
here as implemented.
These adapters fill Component 6 for THIS repo only. Other instances will pick
differently; see `../03-research/01-architecture/02-components/spec-graph/spec-graph.md` for the candidate catalog.
...
The flat `.eposforge/scripts/` directory is not permitted; nothing may live
...
which runs from the `pre-commit` hook fragment owned by `source-control-ci`
...
fragments at
`.eposforge/<component>/scripts/hooks/<git-hook-name>` and a single
composer at
...
3. **Indexing / extraction:**
  ...
  entities and relationships into cognee's embedded Ladybug/Kuzu-fork
  graph + LanceDB vector store. Before `cognify`, cognee-sync runs the
  Component 10 budget preflight gate; deny exits with status 4. After
  successful `cognify`,
  ...
  `adapter.invoked` token-usage event.
   - Opt-in fallback path (`--graphrag`): GraphRAG reads all `*.md` files
     matching
     `^(00-vision|01-architecture|02-roadmap|03-research|\.eposforge|\.eposforge/adrs)/.*\.md$`
     from the repo root and produces Parquet outputs under
     `.eposforge/spec-graph/graphrag/output/`.

4. **Import / projection to query store:**
   - Default path: cognee writes directly into its own embedded Ladybug/Kuzu-fork + LanceDB
...
5. **Query surface:** Cognee's MCP server (`dkr-cgnee-mcp`, in proxy mode
  ...
  `.eposforge/spec-graph/cognee/scripts/adopter-recall.py` to
  enforce adopter-boundary path rewriting (no raw `.eposforge/...`
...
6. **Rebuild flag:** The `.eposforge/spec-graph/scripts/hooks/post-commit`
   hook fragment (composed into `.git/hooks/post-commit` by the SCM/CI adapter's
   `install-hooks.sh`) writes `.eposforge/spec-graph/.needs-rebuild`
...
| Output | Description |
|---|---|
| Cognee embedded store on `dkr-cgnee-api` | Authoritative KG: Ladybug/Kuzu-fork graph (`cognee_graph_ladybug`) + LanceDB vector indexes under `./data/cognee_system/databases/` |
| `.eposforge/spec-graph/cognee/sync/.cognee-state.db` | SQLite state store: `file_path → (dataset_id, data_id, content_hash)` committed to source |
| `.eposforge/spec-graph/graphrag/output/*.parquet` | Fallback only: Entity / Relationship / Community / TextUnit tables |
| Neo4j graph | Fallback only: target of the opt-in GraphRAG import |
| `.eposforge/spec-graph/.needs-rebuild` | Flag file set by post-commit hook |
...
| `cognee-sync` (uv project) | local — `.eposforge/spec-graph/cognee/sync/` | Incremental git-diff-driven sync CLI | default |
...
| Privacy (graph) | `local` — cognee's embedded Ladybug/Kuzu-fork + LanceDB store lives on the API container's volume and is never sent to a vendor (default path); Neo4j on operator's machine (fallback) |
...
| No output dir | `.eposforge/spec-graph/graphrag/output/` absent | GraphRAG fallback `import.sh` exits 1 with error message |
| No venv | `.eposforge/spec-graph/graphrag/.venv/` absent | GraphRAG index/import scripts exit 1 with setup instructions |
...
- `.eposforge/spec-graph/graphrag/settings.yaml` (any key that changes observable behavior)
- `.eposforge/spec-graph/graphrag/scripts/rebuild.sh`
- `.eposforge/spec-graph/cognee/sync/src/cognee_sync/` (cognee-sync CLI core)
- `.eposforge/spec-graph/graphrag/scripts/index.sh`
- `.eposforge/spec-graph/graphrag/scripts/import.sh`
- `.eposforge/spec-graph/cognee/scripts/cognee.py`
- `.eposforge/source-control-ci/github-and-actions/scripts/check-doc-classification.py` (regulated directories, exempt patterns, or required fields)
- `.eposforge/source-control-ci/github-and-actions/scripts/generate-installed-index.py` (adapter crawl logic or index schema)
- `.github/workflows/doc-lint.yml` (trigger paths or job behaviour)
- `.eposforge/spec-graph/scripts/hooks/post-commit`
