...
Usage:
    python.eposforge/tool-transport/mcp-stdio-and-http/scripts/sync-mcp.py
...
#   scripts/ -> mcp-stdio-and-http/ -> tool-transport/ -> installed/ ->.eposforge/ -> repo/
# Wait, it's actually:
#   scripts/ -> mcp-stdio-and-http/ -> tool-transport/ -> installed/ ->.eposforge/ -> repo
# Let's count: scripts is under mcp-stdio-and-http which is under tool-transport
# which is under installed which is under instance which is under repo root.
# So: script.parent = scripts/, .parent = mcp-stdio-and-http/, .parent = tool-transport/,
#     .parent = installed/, .parent =.eposforge/, .parent = repo root
# ---------------------------------------------------------------------------
...
_SECRETS_ROOT = _REPO_ROOT / "instance" / "installed" / "secrets-key-management"
_EPOS_SECRETS = _REPO_ROOT / "instance" / "installed" / "secrets-key-management" / "bin" / "epos-secrets"
...
def main() -> int:
    ...
    if check_mode:
        ...
        if drift:
            print(
                "\nRun: python.eposforge/tool-transport/mcp-stdio-and-http/scripts/sync-mcp.py",
                file=sys.stderr,
            )
            return 1
        else:
