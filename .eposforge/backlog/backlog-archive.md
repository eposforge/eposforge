...
Resolved: 2026-06-16
Validation (Phase 2 enforce, confirmed 2026-08-15): `config.toml` declares `visibility = "public"`; `lint-backlog.sh` builds the public/private map, ERRORs on public→private cross edges and runs a whole-file leak scan across active, slated and archive; its `--help` documents the rule and the single-root degradation. Verified by the check firing on real leaks during the 2026-08-15 tick-off audit. A duplicate `open` copy of this item was removed from `backlog.md` on the same date — the item is resolved, and one ID must have one status in one file.
...
## Issue EF-075 — Cross-agent review contract: handoff / findings / disposition payloads with a mechanical stop rule
ID: EF-075
Title: Cross-agent review contract: handoff / findings / disposition payloads with a mechanical stop rule
Date: 2026-08-15
Status: resolved
Effort: L
Fix surface: eposforge-pattern
Tags: agent-policy, orchestrator
Verify with: three JSON Schemas exist under the framework's `crosscheck-schemas/` directory (handoff, findings, disposition), each carrying a versioned `schema` string; `validate-payload.sh` exits non-zero for a findings payload whose `claims_verified[]` does not cover every `claims[].id`, for a disposition entry of `accepted-deferred` or `rejected` carrying no `detail`, and for a handoff over its declared token cap; `crosscheck-decide.sh` returns `continue` for fixtures carrying a refuted claim, a non-empty `uncovered_scope[]`, or a blocker/major finding attributed `introduced-by-change`, and `stop` for a clean fixture and for one whose only blocker is attributed `pre-existing`; three runs over one fixture return one distinct answer.
...
Fix surface: eposforge-pattern
Tags: source-control
Validation: Verified 2026-08-30 against the shipped document, not against a commit message. The standard is `04-standards/10-ungameable-gate/ungameable-gate.md`, listed in `04-standards/README.md` and cross-referenced twice from `01-architecture/02-components/source-control-ci.md`. It declares all five anti-gaming requirements (write-scope separation, spec-derived tests, outcome altitude, tamper-evidence, held-out assertions), the dispatch -> execute -> run gate -> diagnose -> re-run loop, done-only-on-gate-pass and never on self-report, and the same gate as a required PR status check; the loop-visibility-vs-ungameable tension and its resolution are stated explicitly. Closed together with a new section added the same day, `Declaring Done When There Is No Gate` (requirements 6-9), which covers the case the original text left open: it governs the *claim* rather than the state, for the common situation where no gate exists yet. Requirement 6 makes a done claim carry a named command actually run in the task; requirement 7 forbids declaring done with no gate covering the changed behavior and puts building the missing check inside the task; requirement 8 names the proxies that may not be offered as evidence (a passing lint or schema validator, a clean build or type-check, a commit or PR whose title names the task, a service reporting itself healthy, a check only ever run on well-formed input, a suite whose condition cannot yet arise, impl-derived tests, and an agent re-reading its own diff); requirement 9 requires the condensed rule in always-loaded agent instructions. Conformance re-run at closure: `rg "done claim" AGENTS.md` returns the mirrored rule in the new `Declaring work done` section (2 hits), and `check-component-links.py --check` prints `component-links: OK`. Recall clause partially met. After `cognee-sync --modified` on the standard, `AGENTS.md`, and the ontology, CHUNKS recall for `definition of done` and for `can agents game the tests` each return this standard as the top hit, with the new section present in the returned chunk. Recall for `reward hacking` does NOT return it -- before or after the term was added to the doc -- and instead returns unrelated planning documents; that one named query is an open retrieval gap, recorded here rather than papered over. Out of scope and still true: this standard predates and does not satisfy `04-standards/00-standards-meta` requirements 2-3 (no frontmatter block, no `Status` section, section order); that is pre-existing and is not part of this item's criteria.
