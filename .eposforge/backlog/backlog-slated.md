...
Verify with: `01-architecture/02-components/working-memory.md` exists as a `source_of_truth: yes` slot contract declaring scope (cross-session / cross-turn agent working memory: conversation state, scratchpad state, decision rationale, partial-work checkpoints) and explicit non-scope (KG / ontology — stays in Spec Graph; per-call vector RAG inside a Spec Graph implementation — stays inside Spec Graph; immutable raw chat capture for distillation — stays in Interaction Capture / EF-023/EF-024); declares a recall / write / decay / eviction API consumed by Dev Product and optionally Orchestrator; declares correlation identifiers (provider account, machine, workspace) compatible with the EF-024 Track 1 chat-event schema; the candidate-implementations catalog lists at minimum Mem0, Zep, LangMem, Letta as adapter options; cross-references in the Dev Product and Spec Graph docs name Working Memory as the working-memory slot and clarify the boundary against Spec Graph (KG only).
Notes: Industry-standard agentic factories typically have a memory layer distinct from the knowledge graph: short-term / working / cross-session conversational memory (Mem0, Zep, LangMem, Letta). Cognee (the current Spec Graph implementation) confirms — and the spec-graph contract supports — that vector indexing and short-term/working/conversational memory are "separate and out of scope" for Spec Graph. Each Dev Product (e.g., Claude Code, Copilot) keeps its own native per-session memory, but nothing in the EposForge contract gives an adopter a deterministic place to plug a cross-session / cross-IDE memory service for agents. Slated rather than open because EF-023 / EF-024 (chat capture) are already producing the *recording* side of the memory pipeline — until those ship and the *read-side* gap becomes concrete (do agents actually need active recall during execution, or is post-hoc operator query sufficient?), a top-level component slot would be speculative. Re-evaluation criterion: once EF-024 is in staging and operators can query indexed chat memory, observe whether agents would benefit from in-loop recall of prior decisions/scratchpads — if yes, promote Working Memory to open; if no, fold the contract into EF-023's schema as a read-side capability. Adjacency: EF-023 / EF-024 (capture is the storage side; Working Memory is the active-recall side), EF-025 (one-KG-vs-many decision — affects whether per-repo memory mirrors the per-repo source-code-KG question), Spec Graph (hard boundary — KG only). Discovered 2026-05-28 during a dark-factory architecture mapping against industry conventions (see an adopter's reference-architecture diagram).
Execution update (2026-08-14): EF-023's capture-contract half and EF-024 Track 1 have landed (Interaction Capture slot + `04-standards/13-chat-event-schema/`). This item's inputs moved: correlation identifiers now have a real schema to be compatible with, and the stale `15-working-memory.md` filename is corrected above to `working-memory.md` (catalog identity is the name, not a number). Re-evaluate-by 2026-08-31 still stands — Track 3 search is not built, so the read-side gap is not yet observable.
