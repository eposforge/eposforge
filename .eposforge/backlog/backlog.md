...
Notes: Filed 2026-08-17. Both scripts read the full `BACKLOG_ROOTS` list but only ever act against the first entry, so every cross-repo link in the remaining roots false-flags as an unknown ID. The documented workaround is to loop, invoking once per root with that root placed first, which multiplies the run and is easy to get wrong. The consequence is worse for `sweep-resolved.sh`, which runs `lint-backlog.sh` unconditionally and aborts on any error: in a set of adopter roots with genuine cross-repo references, the sweep cannot run at all without either the loop workaround or an operator with read access to every root in the set. That makes a `resolved` item unsweepable, and a `resolved` item left in an active file is itself a lint error — so the defect manufactures the error it then refuses to let you clear. Adopters have been living with this by accepting standing lint errors, which erodes the signal that lint is meant to carry. Keep the strictness: a link that resolves nowhere in the set must still fail, or the fix trades a false positive for a false negative.
