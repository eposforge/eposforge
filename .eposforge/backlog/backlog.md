...
## Issue EF-098 — Test runner contract: something unattended runs the suite, at outcome altitude, under a time budget
ID: EF-098
Title: Test runner contract: something unattended runs the suite, at outcome altitude, under a time budget
Date: 2026-09-13
Status: resolved
Effort: S
Fix surface: eposforge-pattern
Tags: source-control
TargetShapeOf: prose-to-contract
Verify with: `01-architecture/02-components/source-control-ci-test-runner.md` requires (8) that something unattended runs the suite and a red result reaches a person, on push, on a schedule, or both; (9) outcome altitude, citing Ungameable Gate requirement 3, a handful of checks per repository; (10) a declared time budget for the unattended run, exceeding it being a failure; (11) an optional short list for scheduled runs in the suite's format; (12) an honest statement that a capability without a check is detectable only where work is declared first and names its covering check; the checks folder is not mandated and `.eposforge/standing-tests/` is suggested; the file names no host and no adopter item ID.
Validation: Landed 2026-09-13 as a971f86. An adopter measured the gap first: five repositories had standing suites, three were red, and nothing ran any of them where a red result reached anyone — including this repository, whose GitHub run had failed for a day unnoticed. The contract had never repeated the outcome-altitude rule, and one adopter's maximal per-capability suite with mutation testing grew past any scheduled budget.
Resolved: 2026-09-13
Notes: Filed and resolved in the same sitting so the contract change has an item.

## Issue EF-099 — A Covered by: backlog field and lint rule, generic
ID: EF-099
Title: A Covered by: backlog field and lint rule, generic
Date: 2026-09-13
Status: open
Effort: M
Fix surface: eposforge-pattern
Tags: backlog-tooling, source-control
TargetShapeOf: prose-to-contract
Verify with: the backlog schema documents an optional `Covered by:` field naming one or more repository-relative standing checks; `lint-backlog.sh` accepts it without an unknown-field warning; when the field is present lint reports a named path that does not exist, is not a live (uncommented) line of `.eposforge/standing-suite` or `.eposforge/standing-nightly`, or does not mention the item ID; a fixture item with a commented-out suite line is reported, not accepted; the schema says plainly that the field detects a missing check only where work is declared before it is built.
Notes: Filed 2026-09-13. After an adopter proves the shape: one adopter already refuses a feature child with no `Covered by:` line in its dispatch path, and has a private check verb for ordinary items. Generalize only once that has run for a while.

## Issue EF-100 — A generic, publishable reference for the standing-suite status and prove tools
ID: EF-100
Title: A generic, publishable reference for the standing-suite status and prove tools
Date: 2026-09-13
Status: open
Effort: M
Fix surface: eposforge-pattern
Tags: source-control, distribution
Verify with: a reference implementation tagged example-not-mandate provides, with no host names, forge addresses or credentials baked in, (a) run a repository's suite from a fresh clone under a budget, (b) prove a new check fails on a base revision and passes on HEAD, refusing a check that was never red, and (c) report the latest push-run result for the default branch's commit from a configured forge; each verb has a command line and a thin MCP front door; its own standing checks run clone-local with no network.
Notes: Filed 2026-09-13. Only once a private adopter instance has run for a while; do not extract early. Adjacency: EF-098 (the contract this serves), EF-099.
