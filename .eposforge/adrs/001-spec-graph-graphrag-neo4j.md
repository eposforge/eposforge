...
- Requires no external hosted services beyond the inference API
  ...
  changes.
  section in `.eposforge/SPEC.md` and choose accordingly.
...
- Custom extraction prompts in `.eposforge/spec-graph/graphrag/prompts/` encode the
...
- Instances with privacy requirements should substitute a local Dev
  ...
  `.eposforge/spec-graph/graphrag/settings.yaml` accordingly.

---

## Decision: Automation — Non-blocking post-commit flag + manual rebuild

**Chosen:** post-commit hook writes `.eposforge/spec-graph/.needs-rebuild` flag;
operator runs `.eposforge/spec-graph/graphrag/scripts/rebuild.sh` after significant batches.
...
- A CI-triggered rebuild (optional extension) adds complexity that is
  ...
  `.eposforge/spec-graph/graphrag/scripts/rebuild.sh` on the CI host.
...
- **Positive:** native Neo4j vector indexes (three: `entity_embedding`,
  ...
  `.eposforge/spec-graph/graphrag/scripts/import.sh` using embeddings sourced from LanceDB
  (`.eposforge/spec-graph/graphrag/output/lancedb`). Hybrid Cypher queries that combine
  structural traversal with semantic similarity are supported without
  any MCP-layer changes. See `.eposforge/spec-graph/graphrag/README.md` for example queries.
