...
.DESCRIPTION
    ...
    declared in:
           .eposforge/execution-sandbox/windows-acl-user/windows-acl-user.md
...
$WorkspaceRoot  = '<abs-path-to-your-eposforge-clone-on-windows>'  # e.g. the workspace clone path
$OperatorProfile = (Join-Path (Join-Path $env:SystemDrive 'Users') $OperatorUsername)  # resolve dynamically (typical users root)
...
$usersRoot = (Join-Path $env:SystemDrive 'Users')  # typical Windows users root (e.g. C:\Users)
...
$runnerProfile = Join-Path (Join-Path $env:SystemDrive 'Users') $RunnerAccount  # e.g. the runner account profile dir under users root
...
Sandbox Living Spec:
    $WorkspaceRoot\instance\installed\execution-sandbox\windows-acl-user\windows-acl-user.md
