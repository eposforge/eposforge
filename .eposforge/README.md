...
#.eposforge/ — EposForge Self-Host Layer (uniform container)
...
**Note (EF-059/060):** This container is now named `.eposforge/` uniformly (was `.eposforge/` in the framework and `eposforge/` in adopters). See 04-standards/07-adapter-layout-mirror.
...
| Spec Graph | filled | Cognee (embedded Ladybug/Kuzu fork + LanceDB; GraphRAG/Neo4j archived snapshot retained only for possible revival) | [SPEC.md](./SPEC.md) · [spec-graph/](./spec-graph/) | [../01-architecture/02-components/spec-graph.md](../01-architecture/02-components/spec-graph.md) |
...
Pick your own adapters for each slot. You can mirror this `.eposforge/`
container layout if it is useful, but these concrete choices are not prescriptive.
