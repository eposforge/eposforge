...
```powershell
pwsh.eposforge/secrets-key-management/sops-age/setup.ps1
```
...
```powershell
python.eposforge/secrets-key-management/bin/epos-secrets -- <command>
```
...
python.eposforge/secrets-key-management/bin/epos-secrets -- bash.eposforge/spec-graph/scripts/rebuild.sh

# Run a specific script with only the secrets it needs
python.eposforge/secrets-key-management/bin/epos-secrets --only ANTHROPIC_API_KEY,NEO4J_PASSWORD -- python my-script.py

# Resolve manifests + vault from another repo while using the same resolver binary
EPOS_SECRETS_HOME=/path/to/adopter/secrets-key-management \
python.eposforge/secrets-key-management/bin/epos-secrets -- <command>
```
...
1. On the new machine, generate a request payload:
   - Linux: `bash.eposforge/secrets-key-management/sops-age/setup.sh`
   - Windows: `python.eposforge/secrets-key-management/bin/epos-machine-request`
...
5. Pull on the new machine, then run `python.eposforge/secrets-key-management/bin/epos-secrets --check`.
