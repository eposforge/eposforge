...
> Slot contract: [../../../01-architecture/02-components/secrets-key-management.md](../../../01-architecture/02-components/secrets-key-management.md)
> Candidate catalog: [../../../03-research/01-architecture/02-components/secrets-key-management/secrets-key-management.md](../../../03-research/01-architecture/02-components/secrets-key-management/secrets-key-management.md)
...
This adapter FULFILLS_SLOT `secrets-key-management` as the primary shared-secrets store.
...
|---|---|
| `name` | `sops-age` |
| `component` | `secrets-key-management` |
| `version` | `0.1.0` |
| `status` | `experimental` |
...
| `resolver` | `.eposforge/secrets-key-management/bin/epos-secrets` |
...
- Resolver location is decoupled from vault location: set `EPOS_SECRETS_HOME` (or
  `EPOS_VAULT`) to point at another repo's `secrets-key-management/` directory
...
- Each resolved secret emits one `secret.accessed` JSON line (no value) to
  `$EPOS_AUDIT_SINK` if set, otherwise `.eposforge/.audit/secret-access.jsonl`.
...
```powershell
python.eposforge/secrets-key-management/bin/epos-machine-request
```

Operator-side approval (Linux or Windows):

```sh
python.eposforge/secrets-key-management/bin/epos-authorize --request-file epos-machine-request.json
```

After the approval commit is pulled, verify on the target machine:

```sh
python.eposforge/secrets-key-management/bin/epos-secrets --check
```
...
cd.eposforge/secrets-key-management/sops-age
