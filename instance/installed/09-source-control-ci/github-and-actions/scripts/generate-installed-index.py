#!/usr/bin/env python3
"""generate-installed-index.py — Generate instance/installed/_index.json.

Crawls every installed adapter under instance/installed/<component>/<adapter>/
and extracts the machine-readable metadata declared in the adapter's Living
Spec (<adapter>.md).  Writes the result to instance/installed/_index.json.
...
Usage
-----
    python instance/installed/09-source-control-ci/github-and-actions/scripts/generate-installed-index.py
    python instance/installed/09-source-control-ci/github-and-actions/scripts/generate-installed-index.py --check
      ...
      "adapters": [
        {
          ...
          "adapter":            "claude-code",
          "spec":               "instance/installed/03-dev-product/claude-code/claude-code.md",
          "scripts_dir":        "instance/installed/03-dev-product/claude-code/scripts",
          "name":               "claude-code",
          "status":             "approved",
...
INDEX_PATH = INSTALLED_DIR / "_index.json"
...
def build_index() -> dict:
    """Crawl installed/ and build the full index dict."""
    adapters = []
...
def main() -> int:
    ...
    if args.check:
        if not INDEX_PATH.exists():
            ...
            print("Run: python instance/installed/09-source-control-ci/github-and-actions/scripts/generate-installed-index.py")
            return 1
        ...
        if _stable_json(committed) != _stable_json(fresh):
            print("FAIL: instance/installed/_index.json is stale.")
            print("Run: python instance/installed/09-source-control-ci/github-and-actions/scripts/generate-installed-index.py")
            # Show a diff hint
            ...
            if removed:
                print(f"  Adapters removed from installed/: {sorted(removed)}")
