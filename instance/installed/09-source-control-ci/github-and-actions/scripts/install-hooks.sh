...
#   instance/installed/<component>/<adapter>/scripts/hooks/<git-hook-name>
#
...
# instance/installed/*/scripts/hooks/ and instance/installed/*/*/scripts/hooks/.
...
status=0
for fragment in \
  "$repo_root"/instance/installed/*/scripts/hooks/"$hook_name" \
  "$repo_root"/instance/installed/*/*/scripts/hooks/"$hook_name"; do
  if [ -x "$fragment" ]; then
    "$fragment" "$@"
  else
    bash "$fragment" "$@"
...
case "$MODE" in
  install)
    ...
    discover_fragments | awk -F'\t' \
      '{ sub(/^.*\/instance\/installed\//, "instance/installed/", $2); printf "  %s\t%s\n", $1, $2 }'
    ;;
  check)
