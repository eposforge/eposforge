...
if [ "$USE_COGNEE" = true ]; then
  ...
  "${PYTHON}" "${REPO_ROOT}/installed/06-spec-graph/cognee/scripts/cognee.py"
else
  bash "${REPO_ROOT}/installed/06-spec-graph/graphrag/scripts/index.sh"
  echo ""
  bash "${REPO_ROOT}/installed/06-spec-graph/graphrag/scripts/import.sh"
fi
