...
#   python instance/installed/12-secrets-key-management/bin/epos-secrets -- bash instance/installed/06-spec-graph/graphrag/scripts/rebuild.sh
#
# Legacy invocation (manual env export):
#   ANTHROPIC_API_KEY=xxx NEO4J_PASSWORD=zzz bash instance/installed/06-spec-graph/graphrag/scripts/rebuild.sh
set -euo pipefail
...
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" ]]; then
  GRAPHRAG_PYTHON="${REPO_ROOT}/instance/installed/06-spec-graph/graphrag/.venv/Scripts/python"
else
  GRAPHRAG_PYTHON="${REPO_ROOT}/instance/installed/06-spec-graph/graphrag/.venv/bin/python"
fi

if [[ ! -f "${GRAPHRAG_PYTHON}" ]]; then
  echo "ERROR: GraphRAG venv not found. Expected: $(dirname "${GRAPHRAG_PYTHON}")/../.venv" >&2
  echo "Run: cd instance/installed/06-spec-graph/graphrag && python -m venv .venv && .venv/bin/pip install 'graphrag==3.0.9' neo4j pandas pyarrow" >&2
  exit 1
fi

GRAPHRAG_OUTPUT_DIR="${REPO_ROOT}/instance/installed/06-spec-graph/graphrag/output"
GRAPHRAG_CACHE_DIR="${REPO_ROOT}/instance/installed/06-spec-graph/graphrag/cache"
