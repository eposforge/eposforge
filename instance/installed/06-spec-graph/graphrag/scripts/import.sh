...
# Parquet files from installed/06-spec-graph/graphrag/output/ and batch-imports them into Neo4j.
...
#   - Python venv at installed/06-spec-graph/graphrag/.venv with neo4j, pandas, pyarrow installed.
...
if [[ ! -d "${OUTPUT_DIR}" ]]; then
  echo "ERROR: GraphRAG output directory not found: ${OUTPUT_DIR}" >&2
    echo "Run instance/installed/06-spec-graph/graphrag/scripts/index.sh first." >&2
  exit 1
fi
