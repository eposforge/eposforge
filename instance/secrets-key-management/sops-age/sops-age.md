...
| `resolver` | `instance/secrets-key-management/bin/epos-secrets` |
...
```sh
bash instance/secrets-key-management/sops-age/setup.sh
```
...
```powershell
python instance/secrets-key-management/bin/epos-machine-request
```

Operator-side approval (Linux or Windows):

```sh
python instance/secrets-key-management/bin/epos-authorize --request-file epos-machine-request.json
```

After the approval commit is pulled, verify on the target machine:

```sh
python instance/secrets-key-management/bin/epos-secrets --check
```
...
cd instance/secrets-key-management/sops-age
