...
```powershell
python instance/secrets-key-management/bin/epos-secrets -- <command>
```
...
python instance/secrets-key-management/bin/epos-secrets -- bash instance/spec-graph/scripts/rebuild.sh

# Run a specific script with only the secrets it needs
python instance/secrets-key-management/bin/epos-secrets --only ANTHROPIC_API_KEY,NEO4J_PASSWORD -- python my-script.py
...
1. On the new machine, generate a request payload:
   - Linux: `bash instance/secrets-key-management/sops-age/setup.sh`
   - Windows: `python instance/secrets-key-management/bin/epos-machine-request`
...
5. Pull on the new machine, then run `python instance/secrets-key-management/bin/epos-secrets --check`.
