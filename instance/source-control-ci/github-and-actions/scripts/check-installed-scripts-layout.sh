...
# under instance/<component>/scripts/ (or .../<adapter>/scripts/).
# The flat instance/scripts/ directory is not permitted.
#
# Invoked from:
#   * pre-commit hook fragment at
#     instance/source-control-ci/github-and-actions/scripts/hooks/pre-commit
...
FORBIDDEN_DIR="${REPO_ROOT}/instance/scripts"
...
if [ "${#offenders[@]}" -gt 0 ]; then
  cat >&2 <<'MSG'
ERROR: forbidden files under instance/scripts/.
...
The flat instance/scripts/ directory is not permitted. See AGENTS.md
§Conventions and instance/SPEC.md §"Script placement convention".
