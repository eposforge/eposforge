#!/usr/bin/env python3
"""generate-installed-index.py — Generate instance/_index.json.

Crawls every installed adapter under instance/<component>/<adapter>/
and extracts the machine-readable metadata declared in the adapter's Living
Spec (<adapter>.md).  Writes the result to instance/_index.json.
...
Usage
-----
    python instance/source-control-ci/github-and-actions/scripts/generate-installed-index.py
    python instance/source-control-ci/github-and-actions/scripts/generate-installed-index.py --check
      ...
      "adapters": [
        {
          ...
          "adapter":            "claude-code",
          "spec":               "instance/dev-product/claude-code/claude-code.md",
          "scripts_dir":        "instance/dev-product/claude-code/scripts",
          "name":               "claude-code",
          "status":             "approved",
...
REPO_ROOT = Path(__file__).resolve().parent.parent.parent.parent.parent  # scripts -> github-actions -> source-control-ci -> instance -> eposforge (repo root)
INSTALLED_DIR = REPO_ROOT / "instance"  # flat (no /installed subdir); backlog adapter lives under the backlog/ data dir
INDEX_PATH = REPO_ROOT / "instance" / "_index.json"
...
def build_index() -> dict:
    """Crawl instance/ (flat component dirs) and build the full index dict."""
    adapters = []
...
def main() -> int:
    ...
    if args.check:
        if not INDEX_PATH.exists():
            ...
            print("Run: python instance/source-control-ci/github-and-actions/scripts/generate-installed-index.py")
            return 1
        ...
        if _stable_json(committed) != _stable_json(fresh):
            print("FAIL: instance/_index.json is stale.")
            print("Run: python instance/source-control-ci/github-and-actions/scripts/generate-installed-index.py")
            # Show a diff hint
            ...
            if removed:
                print(f"  Adapters removed from instance/: {sorted(removed)}")
