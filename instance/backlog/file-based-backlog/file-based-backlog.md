...
- **Multi-graph model**: Each major scope (EposForge pattern, adopter's primary repo / Adopter Platform Spec, product repos, backlog) can maintain its own graph. Shared ontology terms provide the mapping between them. The adopter's primary repo (e.g. adopter) is the single repo containing overall eposforge implementation documentation for both factories plus the `eposforge/` slice; portfolio reviews are run from there. (See capture/plan for terminology and EF-056.)
