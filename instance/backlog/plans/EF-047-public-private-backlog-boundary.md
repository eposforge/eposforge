...
| # | Leak | Where | Kind |
|---|------|-------|------|
| 1 | `Blocks:` line pointing at two GEA adopter IDs | EF-032 | **structural edge** (worst — shows up in `--graph`, ingested as a relationship) |
...
| 3 | "GEA LAN" in title + verify-with | EF-023, EF-024 | prose / scope |
| 4 | named adopter example "GraceEnterprises … GEA copy" | EF-022 notes | prose |
...
1. **EF-032 structural edge** — delete the `Blocks: GEA-…` line. Lossless: the
...
3. **EF-022 prose** — "GraceEnterprises single-vault migration" / "the duplicated
   GEA copy" → "an adopter's single-vault migration" / "the duplicated adopter
   copy."
4. **EF-023 / EF-024 — DECISION PENDING (do not execute blind):** these are scoped
   to "GEA LAN" in their *titles*, which may mean they are adopter (GEA) work
   ...
   - **sanitize-in-place** — keep as a framework observability/memory pattern;
     "GEA LAN" → "the adopter's LAN" / "LAN-local"; **or**
   - **relocate** — move EF-023/EF-024 into the GEA backlog as `GEA-` items.
   Phase 1 sanitizes #1–#3 immediately; #4 waits on this call.

**Phase 1 verify:** `grep -nE 'GEA|GraceEnterprise|/mnt/raid-storage|\.lan'
instance/backlog/backlog.md` returns only intentional matches (e.g. operational
...
- GEA, IAC, OAPI, OA, EXR (adopters) → `private`
