...
| 4 | named adopter example "GraceEnterprises … adopter copy" | EF-022 notes | prose |
...
3. **EF-022 prose** — "GraceEnterprises single-vault migration" / "the duplicated
   adopter copy" → "an adopter's single-vault migration" / "the duplicated adopter
   copy."
4. **EF-023 / EF-024 — DECISION PENDING (do not execute blind):** these are scoped
   to "adopter LAN" in their *titles*, which may mean they are adopter (adopter) work
...
**Phase 1 verify:** `grep -nE 'adopter|GraceEnterprise|/mnt/raid-storage|\.lan'
...
- adopter, IAC, OAPI, OA, EXR (adopters) → `private`
