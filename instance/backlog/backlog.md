...
Status: resolved
Effort: L
Theme: backlog-tooling, simplification
Fix surface: eposforge-pattern
Verify with: adapters live at `instance/<stable-name>/<adapter>/` (e.g. `instance/backlog/file-based-backlog/scripts/`, `instance/dev-product/...`); no numeric prefixes anywhere on component dirs; `_index.json` at instance/_index.json (regenerated); adapter-layout-mirror and scripts updated; adopter adopter container also migrated; layout check/generate pass. The numbered folder scheme is retired.
