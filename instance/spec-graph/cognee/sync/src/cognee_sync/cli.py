...
def _validate_inference_routing() -> None:
    ...
    validator = _script_path(
        "instance/inference/scripts/validate-azure-routing-config.sh"
    )
...
def _run_budget_gate(repo_key: str, requested_tokens: int, model: str) -> dict[str, object]:
    checker = _script_path("instance/inference/scripts/check-budget-gate.sh")
    proc = subprocess.run(
        [
            "bash",
            checker,
...
def _record_budget_usage(repo_key: str, consumed_tokens: int) -> None:
    recorder = _script_path("instance/inference/scripts/record-budget-usage.sh")
    proc = subprocess.run(
        [
            "bash",
            recorder,
...
def _emit_token_usage_event(
    ...
    emitter = _script_path("instance/inference/scripts/emit-token-usage-event.sh")
    proc = subprocess.run(
        [
            "bash",
            emitter,
