...
#
# Usage:
#   bash instance/spec-graph/cognee/scripts/bulk-rebuild.sh [--dry-run]
#
# What this does:
...
# first perform the KG wipe (see instance/spec-graph/cognee/MAINTENANCE.md
...
SYNC_DIR="${REPO_ROOT}/instance/spec-graph/cognee/sync"
...
if command -v epos-secrets >/dev/null 2>&1; then
  ...
  SECRETS_BIN="${REPO_ROOT}/instance/secrets-key-management/bin/epos-secrets"
fi
...
#   raw EF-/adopter- item text stays in the independent file-based backlog graph)
mapfile -t FILES < <(
  cd "$REPO_ROOT" &&
  git ls-files '*.md' '*.ttl' |
