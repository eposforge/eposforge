...
#   ANTHROPIC_API_KEY=your-key OPENAI_API_KEY=your-key bash instance/spec-graph/graphrag/scripts/index.sh
set -euo pipefail
...
if [[ ! -d "${VENV}" ]]; then
  ...
  echo "Run: cd instance/spec-graph/graphrag && python -m venv .venv && source .venv/bin/activate && pip install graphrag" >&2
  exit 1
fi
...
echo "    Run instance/spec-graph/graphrag/scripts/import.sh to load into Neo4j."
