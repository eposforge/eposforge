...
8. **Consume checklist.** An agent that installs or tells an operator
   ...
   4. Treat an installed script-calling skill as **not installed**
      when its documented home variable is unset and the skill
      cannot otherwise resolve the clone it wraps
      (`skills/INSTALL.md`).

9. **Program-first wrapping.** When the designation test in requirement 4
   picks **skill** or **runbook** and the procedure's steps are known, the
   skill or runbook MUST name the program that holds those steps and
   invoke it via that program's documented home variable — never a
   cwd-relative path. The skill body is then when / dry-run / gate /
   verify; it MUST NOT be the only place the steps exist. Platform or
   substrate lifecycle with a human gate (setting a secret, an elevation
   prompt, applying a compose change) MAY remain a skill wrapping a
   program that a human runs — the exception is the human gate, not
   "the steps are cheaper to keep in prose." Judgment skills (elicitation,
   prompt refinement, the portfolio-review judgment steps) stay skills,
   as prose. Where a judgment skill has a mechanical half, that half is a
   program: the docs-lint pair — a deterministic link checker as the
   floor, the skill carrying the semantic half — is the model. See
   [program-first-procedures](../15-program-first-procedures/program-first-procedures.md)
   for the vehicle-class table, the home-variable invocation contract,
   and gates G1-G4 that enforce this requirement mechanically.
...
- Verify the program-first wrapping requirement is stated:
  `rg -n "Program-first wrapping" 04-standards/03-agent-skills/agent-skills.md`
- Verify `skills/INSTALL.md` states the unset-home-variable rule:
  `rg -n "not installed" skills/INSTALL.md`
...
- [adapter-pattern](../../01-architecture/00-adapter-pattern/adapter-pattern.md) —
  ...
  chooses.
- [program-first-procedures](../15-program-first-procedures/program-first-procedures.md) —
  the program-first wrapping contract requirement 9 feeds into: vehicle
  classification, the home-variable invocation contract, gates G1-G4.
