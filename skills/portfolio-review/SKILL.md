...
- When Gas Town is the orchestrator adapter: knowledge of the spawn path
  (`backlog-to-beads.sh` / skill `gastown-backlog-to-beads`, GEA-007 boundary)
...
1. Mint beads with the GEA-007 spawn rule (dry-run first, then apply only for
...
5. Agent-discovered findings stay in Beads; promote-back to the file backlog
   is human-gated (do not write GEA-/EF- items yourself unless asked).
