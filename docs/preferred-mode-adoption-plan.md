...
Adopter (GEA, IAC, OutreachApi, OutreachAssistant):
...
**SECURITY NOTE (portfolio.md data leak):** `--mermaid` with mixed public+private BACKLOG_ROOTS produces a diagram containing private adopter items (GEA-*, IAC-*, etc.). The writer in aggregate.sh now *deterministically refuses* to place such a diagram under any public (`visibility=public`) root: it selects the first private root from the list instead (so a list starting with the framework will write to GEA's backlog/portfolio.md when GEA is first private). Pure public runs still target the framework. The public `.gitignore` (portfolio.md + **/portfolio.md) remains the final belt-and-suspenders. Never manually copy a mixed diagram into the gh/eposforge tree. Run framework-only (no private roots) when you want the public view.
