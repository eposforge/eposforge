...
1. **Preferred mode, not Vendored-copy.** Scripts run from the framework clone via
   `BACKLOG_HOME=<framework>/instance/installed/13-backlog/file-based-backlog`.
...
2. **One container per adopter, named `eposforge/`.** Everything an adopter takes
   ...
   self-duplicate. Adopters **must not** reproduce the `instance/installed/`
...
Adopter (adopter, IAC, OutreachApi, OutreachAssistant):

```text
<repo>/
  eposforge/                                   # the one eposforge container
    backlog/
      config.toml                              # prefix + fix_surfaces + themes
      backlog.md                               # active: open / in-progress / blocked
      backlog-slated.md                        # deferred
      backlog-archive.md                       # resolved
      backlog-archive-index.md                 # searchable archive index
      portfolio.md                             # generated by aggregate.sh (not seeded; see .gitignore + leak warning below)
    # Living Spec (eposforge/13-backlog/file-based-backlog/file-based-backlog.md)
    #   is part of the model but DEFERRED — not created in this rollout (§8.2).

**SECURITY NOTE (portfolio.md data leak):** `--mermaid` with mixed public+private BACKLOG_ROOTS produces a diagram containing private adopter items (adopter-*, IAC-*, etc.). The writer in aggregate.sh now *deterministically refuses* to place such a diagram under any public (`visibility=public`) root: it selects the first private root from the list instead (so a list starting with the framework will write to adopter's backlog/portfolio.md when adopter is first private). Pure public runs still target the framework. The public `.gitignore` (portfolio.md + **/portfolio.md) remains the final belt-and-suspenders. Never manually copy a mixed diagram into the gh/eposforge tree. Run framework-only (no private roots) when you want the public view.
    # no scripts/ (Preferred mode), no README/adrs/.audit (conveniences)
  <repo>.code-workspace                        # folders: ./eposforge + ../../gh/eposforge
  <repo's own content, untouched>
```
...
eposforge/                                     # the framework repo
  instance/
    ...
    installed/13-backlog/file-based-backlog/   # the adapter (scripts live here)
...
```bash
export BACKLOG_HOME=/mnt/raid-storage/src/git/gh/eposforge/instance/installed/13-backlog/file-based-backlog
...
> framework's `instance/installed/` **adapter** slot for full symmetry is
...
- `AGENTS.md` (lines ~209–216): `backlog/backlog.md` → `instance/backlog/backlog.md`,
  ...
  load lines. (The `instance/installed/.../scripts/...` invocation lines are already
...
- No adopter contains `instance/installed/`, `adrs/`, `.audit/`, or a vendored
  `scripts/` directory.
