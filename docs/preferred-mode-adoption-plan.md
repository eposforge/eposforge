...
- **Adopter repos in scope:** the primary adopter (adopter), IAC, OutreachApi, OutreachAssistant
...
adopter. adopter and IAC already hold backlog data and get *completed*; OutreachApi
...
Adopter primary repo (e.g. adopter — the Adopter Platform Spec):
...
```text
<repo>/                                        # e.g. the primary adopter (primary)
  ...
  eposforge/                                   # the adopted pattern slice (one container)
    backlog/
      config.toml                              # prefix=adopter, visibility=private
...
|---|---|---|---|---|---|---|
| **adopter** | `local/the primary adopter` | ✅ `eposforge/` | ✅ (prefix `adopter`) | ✅ | ❌ broken (`.` + doubled `../../gh/eposforge/eposforge`) | **Complete** |
...
Repo prefixes (settled): `adopter`, `IAC` (existing); **`OAPI`** (OutreachApi);
**`OA`** (OutreachAssistant).

---

## 5. Phase A — Complete adopter & IAC (they already hold data)
...
### A2. adopter — repair the workspace file
`the primary adopter/GraceEnvironment.code-workspace` folders →
```json
...
```
Seed each `.md` with the same header style as IAC/adopter's files (title + one-line
...
1. **Prefixes** — OutreachApi = `OAPI`, OutreachAssistant = `OA` (adopter/IAC unchanged).
...
- adopter, IAC, OutreachApi, OutreachAssistant each: scripts run from the framework clone
...
- A2: `the primary adopter/GraceEnvironment.code-workspace` repaired — done.
...
- adopter: lint finds `eposforge/backlog/` via workspace; items visible (pre-existing cross-repo ref errors are data issues).
