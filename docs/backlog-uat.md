UAT 1 — Lint rule: Status: blocked without an open dep
  ...
  ---
  UAT 5 — Critical-path against a adopter anchor (multi-root)

  BACKLOG_ROOTS="/mnt/raid-storage/src/git/gh/eposforge:/mnt/raid-storage/src/git/
  local/the primary adopter/eposforge" \
    bash instance/backlog/file-based-backlog/scripts/aggregate.sh
  --critical-path adopter-013

  Expect: a chain ending at adopter-013, with the EF-043 node shown as resolved (not
  open), and adopter-013 marked [workable now].
  ...
  vendors the scripts — adopter is data-only — so a sandbox is the way to test this.)
