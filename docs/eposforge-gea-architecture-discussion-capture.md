# EposForge / adopter Architecture Discussion Capture (2026-06)
...
- For adopters like adopter: How to use the upstream EposForge Cognee graph (for pattern) while maintaining its own graph (for current implementation state)? Should adopter implement Living Spec first?
...
1. Backlog *system structure* (schema, load rules, aggregation, roles) belongs in the Spec Graph. Individual backlog *items* (EF-/adopter- text) should not.
...
3. adopter (adopter) needs to leverage the same projection system for grounding dev on EposForge + adopter's current state.
4. Smell: EposForge Cognee graph includes far more than Living Specs.
5. Smell: Living Spec contract assumes single `SPEC.md` per deliverable, but EposForge and adopter are effectively entire distributed living specs.
6. Layering failure: adopter and IAC are for *eposforge platform factory implementation*. Product repos are for product factory. No single repo should "implement EposForge in its entirety." This is underspecified.
7. adopter conflation: adopter is "kind of like the platform factory spec." The actual implementation is srv-docker-hp + other grace.lan components/config/IaC. "EposForge adopter's implementation" mixes the two.

Additional context:
- EposForge = pattern + reference implementations (in `instance/`).
- adopter = the adopter's primary repo (Adopter Platform Spec) that documents the overall eposforge implementation for both factories and contains the eposforge/ slice (with adopter- backlog items, private visibility in aggregation).
- Existing partials: backlog adapter has `role = "substrate"` vs "product"; `aggregate.sh` handles private roots (writes to first private like adopter); ontology distinguishes PlatformFactory/ProductFactory; platform vs product phase roadmaps.

## 3. Encapsulation & Layout Problems (adopter's eposforge/ Folder)
Detailed inspection:
- adopter repo root has `eposforge/` (intended single container per Adapter Layout Mirror standard): backlog/, router/gastown/ (config+scripts), secrets-key-management/, backup-resilience/.
- But implementation bleeds heavily:
  - adopter top-level: `00-north-star/`, `01-reference-architecture/`, `03-standards/`, `04-runbooks/`, `07-project-portfolio/`, `hardware/`, `servers/containers/` (actual docker-compose, Dockerfile, entrypoints for dkr-gstwn-01/Gas Town, cognee, gitea, etc.), `skills/` (gastown-*), `services/`, `instance/`.
  - adopter README positions the *whole repo* as source of truth for servers/containers/networks/storage.
...
- Both EposForge and adopter use numbered folders for knowledge org (EposForge: 00-vision, 01-architecture, 02-roadmap, 03-research, 04-standards + subs; adopter: 00-north-star, 01-reference-architecture, 03-standards, 04-runbooks, 07-project-portfolio, 99-archive + subs).
...
Adapter Layout Mirror is too narrow for full single-repo platform adopters like adopter.

## 4. Key Proposals & Evolution of Thinking
- **Broader adopter repo layout standard needed**: For "single repo like adopter." Current mirror only covers the adopted slice. Need guidance on overall shape while distinguishing platform vs product.
- **Targeted (not total) symmetry/mirroring**:
  ...
  - Do *not* force full repo isomorphism (different purposes: framework = pattern docs + refs; adopter = full platform factory + IaC + own decisions).
- **Mapping layer**:
  ...
  - Graphs contain instantiated relationships (e.g., adopter gastown adoptsFrom EposForge reference).
...
- **Multi-graph architecture** (core model):
  - One graph per scope: EposForge (pattern + references), adopter (adoption + implementation), IAC, each product repo.
...
- **Agent access model** (critical clarification):
  - Ideal: Agents route primarily/exclusively through Cognee MCP / relevant Spec Graphs. No broad raw file access to EposForge, adopter, or other repos (security, isolation, control via sandbox + agent policy + tool transport).
...
- **Overall target layering** (from boundaries note, extended):
  ...
  4. **Adopter Implementation**:
     - adopter repo: living spec of platform adoption (adopted components, custom adapters, decisions, adopter- items).
...
- adopter `eposforge/` encapsulation fails (bleed everywhere + LAN).
...
- Terminology overload around "adopter" and "adopter implementation."
...
12. Per-scope dataset/instance bootstrap for adopter/IAC/products.
...
- adopter-specific: `the primary adopter/eposforge/`, servers/, hardware/, etc. (examples of bleed).
