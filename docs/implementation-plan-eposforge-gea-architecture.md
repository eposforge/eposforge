# Implementation Plan: EposForge / adopter Architecture & Multi-Graph (2026-06) — Zoomed Out

**Source**: Consolidated from full discussion capture in `docs/eposforge-gea-architecture-discussion-capture.md`.

**Goal**: Clarify boundaries, enable multi-graph + ontology-as-mapping, targeted mirroring for humans/extraction, reliable projection, and graph-first agent access while keeping disk canonical. Use strangler fig pattern (see 02-roadmap/adoption-strategy.md) for incremental rollout so we don't lose track.
...
- Checkpoint: portfolio-review run that surfaces this as a theme.
...
- Pilot on EposForge's own backlog + adopter as one adopter.
...
- Stand up adopter's graph (ingest its Living Specs/adoption docs).
- Shared ontology as mapping layer (add/ use adoptsFrom, fulfillsSlot for realizations).
- Agent grounding rules: "pattern graph for EposForge model, local graph for adopter state".
...
**Milestone**: adopter can ground on both graphs cleanly. Ontology bridges them.
...
- Apply lightly to adopter filesystem and containers (for discoverability and better extraction).
...
- Regular visibility in adopter/EF backlogs.
...
1. Expand or companion to Adapter Layout Mirror:
   ...
   - Update for single-repo platform adopters (adopter-style) while preserving "does not govern own application layout" where appropriate.
2. Apply to adopter (targeted):
   - Align `eposforge/` more closely where it makes sense.
   - Improve naming in `servers/`, `hardware/`, container defs.
   - Keep adopter's natural numbering for its knowledge but document mappings.
...
2. Update tooling:
   ...
   - Runbooks for "how adopter agents ground".
3. Address current reality: Gradually restrict broad cross-repo file access (via policy, mounts, execution sandbox).

**Success**: Agents do not need (and ideally cannot) do broad file RAG across EposForge/adopter. No contradiction with Cognee routing. "Search twice" eliminated via reliable graphs + mappings.
...
- Terminology: Introduce "Adopter Platform Spec (adopter repo)" vs "Platform Instance (srv-docker-hp + IaC)" in ontology, vision, EF notes.
...
- Apply to adopter + IAC first, then products.
...
**Phase 0 status (2026-06-28)**: Tracked by EF-056 (master) + children (EF-057 ingestion boundaries + GraphRAG layer pilot, EF-058 terminology/roles, more to follow). The four files are the maintained record. Implementation edits now proceed only against the backlog items and the four files (or as explicitly called for by a child EF-). Existing file-based backlog pieces (aggregate, explicit markup, portfolio-review) are treated as partial alignment / foundation. See capture for full context.

Update this plan (and the capture) after each significant step or portfolio checkpoint.