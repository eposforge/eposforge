...
- For each component, the operator has either:
...
- The [Tool Transport] exposes the required minimum
...
- [Source Control + CI] enforces Standard 11:
...
## Phase D — Orchestrator v0

**Goal:** the [Orchestrator] can take a Spec Input and dispatch one sub-task
to a chosen Dev Product Adapter, end-to-end.

By the end of Phase D:

- The [Orchestrator] accepts normalized Spec Input output and
  produces a sub-task descriptor.
- The [Orchestrator] selects a Dev Product Adapter using metadata + Agent
  Policy.
- The [Orchestrator] dispatches via Tool Transport in an Execution Sandbox.
- The [Orchestrator] opens a PR with the resulting change and the paired
  Living Spec update.
- An end-to-end demo: spec → Orchestrator → Dev Product → MCP-driven (or
...
**Goal:** the Spec Graph is operational; the [Orchestrator] consults it
during decomposition and Adapter selection.

By the end of Phase E:

- The [Spec Graph] Adapter indexes every Living Spec.
- A post-merge hook re-projects affected deliverables.
- The [Orchestrator] queries the graph for reuse and dependency detection
