...
### Cognee (Spec Graph Adapter)

- **Type:** ontology-grounded extraction + embedded graph/vector
  store + first-party MCP.
- **Cost tier:** free OSS + inference / embedding keys.
- **Query languages:** Cognee search / recall MCP tools; not Cypher
  on the default path.
- **Capabilities:** projects markdown Living Specs into an
  ontology-grounded graph; incremental file sync; semantic +
  graph retrieval for reuse, dependency, and impact questions.
- **Notes:** Default Spec Graph extraction engine in the
  framework instance (see `.eposforge/adrs/002-spec-graph-cognee-default.md`).
  **Does not fulfill [Working Memory].** The same product MAY also
  offer a Working Memory Adapter; that Adapter requires a
  **separate dataset** and a distinct Tool Transport route. Do not
  ingest operational memories into the Living Spec dataset, and do
  not answer "what did we remember?" from this Adapter. Working
  Memory candidates live in
