...
the [Orchestrator] dispatches, how secrets are bound, and what each Dev Product
...
|---|---|---|
| **[Orchestrator]** | IDE-native fleet manager (VS Code agents, Cursor background agents) may serve as the Orchestrator. | Own headless Orchestrator required. |
| **[Audit & Observability]** | **Required, live** — the supervision surface. | Required for forensics. |
| **[Agent Policy]** | Permissive-but-bounded, human as backstop; **policy is calibrated here**. | Calibrated policy enforced headlessly. |
| **[Execution Sandbox]** | Recommended; human backstop tolerates lighter isolation. | Required. |
| **[Dev Product] (auth)** | Subscription / OAuth permitted. | **BYOK / API-key required** for ToS-restricted products. |
