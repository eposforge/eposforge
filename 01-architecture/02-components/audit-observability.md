...
- **Is not:** [Interaction Capture]. Audit & Observability records
  **that** factory events happened (logs, metrics, traces, required
  event types above). Interaction Capture holds **what was said** —
  conversation content, tool events, and reconstructable
  `(input → output)` pairs, with corpus semantics (training
  eligibility, additive retention) this slot has no vocabulary for.
