...
- Define minimum content: purpose, observable behavior, inputs /
  outputs, dependencies, non-functional bounds, versioning policy, and
  `operating_inference` (`none` | `on-demand-judgment` | `continuous-loop`)
  — required for every Living Spec. When the value is `continuous-loop`,
  `operating_inference_reason` (a written reason a program cannot hold
  the operating path) and `operating_inference_budget` (a pointer to a
