...
- **Is not:** an embedding store. Embeddings produced by inference are
  ...
  vector search).
- **Is not:** [Interaction Capture]. Inference serves the model call;
  Interaction Capture records what the call contained (and what came
  back). Token / cost metrics still report to
  [Audit & Observability]; the assembled prompt, tool schemas, and
  conversation body do not.
