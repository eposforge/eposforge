...
<!-- allow-router: this contract documents the deprecated 'Router' alias -->
...
- Select a Dev Product Adapter per sub-task using declared metadata
  (privacy, cost, capabilities), [Agent Policy], and the [Spec Graph] when reuse is possible.
...
- **Is not:** the runtime content-safety enforcement point. The Orchestrator calls [Content Safety], the runtime content-safety enforcement point, to inspect payloads.
