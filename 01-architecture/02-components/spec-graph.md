...
- **Is not:** [Interaction Capture]. Spec Graph projects Living Specs
  (intent). Interaction Capture stores what was said.
- **Is not:** [Working Memory]. Working Memory replaces Dev Product
  native memory (operator/agent operational facts). Spec Graph
  does not fulfill that slot, even when the same product also
  offers a Working Memory Adapter.
