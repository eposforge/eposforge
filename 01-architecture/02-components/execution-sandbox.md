...
- Emit sandbox lifecycle + escape-attempt events to [Audit & Observability].
...
- **Is not:** the orchestrator (that is the Orchestrator) or the policy point
  (that is [Agent Policy]). Agent Policy decides whether an action is permitted; the Execution Sandbox bounds what a permitted action can reach — a denylist at Agent Policy is accident-prevention, the sandbox is the containment boundary.
- **Is not:** a long-lived service host. Sandboxes exist for the
  duration of a sub-task.

## Phased Adoption

The Execution Sandbox is recommended under supervised mode and mandatory once autonomous (human-off-the-loop).
