...
- Provide a **test runner** that satisfies the named sub-contract in
  [source-control-ci-test-runner.md](./source-control-ci-test-runner.md):
  one command proves a standing property in an implementer tree and in
  an isolated orchestrator clone; the same command can cover
  agent-config, platform, and product trees; the implementing agent
  does not own the gate and cannot edit it in the same change; held-out
  assertions are possible; already-shipped behavior is tested when a
  bug is found, not as a corpus backfill. Kernel one-command detection
  is a required property of that runner, not a second harness. The
  runner MUST NOT mandate a named vendor. See the
...
- Run factory-level integration tests as a required PR check where the
  ...
  factory components. Isolation is provided by the [Execution Sandbox]; disposable container environments (e.g., Testcontainers) are
...
- Consult [Agent Policy] for
  ...
  approval.
- Trigger [Spec Graph] re-projection on every merge that
...
- `standing_runner_command` — repository-relative command that proves a
  standing property (test-runner sub-contract).
- `standing_suite_path` — in-tree declaration of that command's checks.
  Default `.eposforge/standing-suite`.
- `ring_promotion_pipeline_refs` — references to the promotion
  ...
  cannot be bypassed. Governed by [Release Rings] and
  [Agent Policy].
...
- **Is not:** the [Living Spec] Adapter itself (that contract defines Spec
...
- **Is not:** the deploy target. Deploy is downstream and is governed
  by the [Release Rings] component. Ring promotion
...
- **Is also:** the owner of the test-runner named sub-contract
  ([source-control-ci-test-runner.md](./source-control-ci-test-runner.md)).
  That file is not a component and MUST NOT be added to the roster.
- **Is not:** a second kernel harness. One-command detection is a
  required property of the runner.
- **Is not:** the cross-agent review contract. Review claims are
  session assertions; standing properties run with no review payload.
