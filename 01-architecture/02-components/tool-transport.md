...
- Expose a defined set of capability categories to Dev Products. The
  ...
  - **working-memory** — recall / remember / forget against the
    [Working Memory] store. Distinct from graph-query: this is the
    shared replacement for each Dev Product's native memory, not a
    Living Spec projection. One implementation, every CLI.
...
A [Dev Product]'s built-in local tools (workspace git, host shell,
local filesystem) do **not** satisfy factory **git**, **graph-query**,
or **working-memory**. Those capabilities MUST be first-class transport
tools against the factory's stores (the factory source-control host,
Spec Graph projections, the Working Memory dataset). Built-in tools MAY
satisfy **fs** and **shell** inside the [Execution Sandbox].

Agent verbs on a factory system of record MUST be exposed as transport
capabilities (MCP tools or equivalent registered functions). A skill
that documents a shell recipe against that store does not fulfill the
capability. See
...
- **Is not:** the runtime content-safety enforcement point. [Content Safety] is the runtime content-safety enforcement point.
